 ...
 */
public class LargestAreaFitFirstPackager extends AbstractPackager {
	...
	public LargestAreaFitFirstPackager(List<Container> containers, boolean rotate3D, boolean footprintFirst, boolean binarySearch, int checkpointsPerDeadlineCheck) {
		super(containers, binarySearch, checkpointsPerDeadlineCheck);
	...
	public LaffResult pack(List<Stackable> containerProducts, Container targetContainer, long deadline, int checkpointsPerDeadlineCheck) {
	...
	public LaffResult pack(List<Stackable> containerProducts, Container targetContainer, long deadline, int checkpointsPerDeadlineCheck, BooleanSupplier interrupt) {
	...
	public LaffResult pack(List<Stackable> containerProducts, Container targetContainer,  BooleanSupplier interrupt) {
		...
		Dimension freeSpace = new Dimension(containerStackValue.getLoadDx(), containerStackValue.getLoadDy(), containerStackValue.getLoadDz());
		...
		while(!containerProducts.isEmpty()) {
			...
			int currentIndex = -1;
			StackValue currentStackValue = null;
			
			// first see if there is a box with full height
			for (int i = 0; i < containerProducts.size(); i++) {
				Stackable box = containerProducts.get(i);

				for (StackValue stackValue : box.getStackValues()) {
					if(stack.getDz() == freeSpace.getHeight() && stackValue.fitsInside3D(freeSpace)) {
						
						if(currentStackValue == null || currentStackValue.getArea() < stackValue.getArea()) {
							currentIndex = i;
						}
					}
				}
			...
			Stack level = new DefaultStack(stack.getContainerStackValue(currentBox.get));
			
			stack.add(levelSpace);
		...
		return new LaffResult(containerProducts, stack);
	}

	protected int getBestBox(Container holder, Dimension freeSpace, List<Stackable> containerProducts) {
		...
		for (int i = 0; i < containerProducts.size(); i++) {
			...
			boolean fits = box.rotateLargestFootprint3D(freeSpace);
