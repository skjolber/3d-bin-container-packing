 ...
 * </ol>
 */
public class LoadAwarePlacementControls2 extends AbstractComparatorPlacementControls<Placement> {
	...
	public LoadAwarePlacementControls2(BoxItemSource boxItems, 
	...
	public Placement getPlacement(int offset, int length) {
		...
		for (int i = offset; i < length; i++) {
			...
			for (Point point3d : points) {
				...
				populatePointSupportees(point3d, pointSupportees, stack.getPlacements());
				...
				for (BoxStackValue stackValue : box.getStackValues()) {
					...
					if(stackValue.isLoadIdenticalBoxOnly()) {
						// see if it is possible to stack on top of an identical box within this point
						
						filterPlacementsSupportersWithIdenticalBox(box, stackValue, point3d, placementSupporters, pointSupporters);
						for (Placement candidate : placementSupporters) {
							// so the only the current box will be stacked on the candidate box, which
							// simplifies things a bit
							if(hasSupporteesWithNonIdenticalBox(candidate, stackValue, stack.getPlacements())) {
								continue;
							}
					...
					if(!populatePlacementSupportees(point3d, stackValue, placementSupportees, pointSupporters)) {
						continue;
					}
