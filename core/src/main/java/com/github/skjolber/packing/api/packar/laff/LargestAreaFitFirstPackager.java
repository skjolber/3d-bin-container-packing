...
public class LargestAreaFitFirstPackager extends Packager {
	...
	public LargestAreaFitFirstPackager(List<Container> containers, boolean footprintFirst, boolean binarySearch, int checkpointsPerDeadlineCheck) {
	...
	public LaffResult pack(List<StackableItem> containerProducts, Container targetContainer, long deadline, int checkpointsPerDeadlineCheck) {
	...
	public LaffResult pack(List<StackableItem> containerProducts, Container targetContainer, long deadline, int checkpointsPerDeadlineCheck, BooleanSupplier interrupt) {
	...
	public LaffResult pack(List<StackableItem> containerProducts, Container targetContainer,  BooleanSupplier interrupt) {
		Container holder = targetContainer.clone();

		ContainerStackValue[] stackValues = targetContainer.getStackValues();
		
		for(ContainerStackValue freeSpace : stackValues) {
			LevelStack stack = new LevelStack(freeSpace);

			ContainerStackValue containerStackValue = stack.getFreeContainerStackValue();
			...
			while(!containerProducts.isEmpty()) {
				...
				int currentIndex = getBestBox(holder, containerStackValue, containerProducts);
	
				if(currentIndex == -1) {
					break;
				}
	
				StackableItem currentBox = containerProducts.get(currentIndex);
				...
				freeSpace = containerStackValue = stack.getFreeContainerStackValue();
			}
		}
	...
	protected int getBestBox(Container holder, ContainerStackValue freeSpace, List<Stackable> containerProducts) {
