 ...
 * <p>
	 * Pressure is defined as {@code weight / area}. A box with
...
public class PlainPackagerPressureConstraintTest extends AbstractPackagerConstraintTest {
	...
	void heavyBoxExceedsPressureLimit() {
		...
		try {
			Box bot = Box.newBuilder().withId("bot")
					.withSize(2, 5, 1).withWeight(11).withMaxLoadPressure(1.0).withRotate2D().build();
	...
	void exactPressureBoundaryAccepted() {
		...
		try {
			Box bot = Box.newBuilder().withId("bot")
					.withSize(2, 5, 1).withWeight(1).withMaxLoadPressure(1.0).withRotate2D().build();
			Box top = Box.newBuilder().withId("top")
	...
	void sameWeightOnLargerBaseAccepted() {
		...
		try {
			Box bot = Box.newBuilder().withId("bot")
					.withSize(11, 5, 1).withWeight(1).withMaxLoadPressure(1.0).withRotate2D().build();
			Box top = Box.newBuilder().withId("top")
	...
	void pressurePropagatesDownChain() {
		...
		try {
			Box a = Box.newBuilder().withId("A")
					.withSize(2, 5, 1).withWeight(20).withMaxLoadPressure(1.0).withRotate2D().build();
