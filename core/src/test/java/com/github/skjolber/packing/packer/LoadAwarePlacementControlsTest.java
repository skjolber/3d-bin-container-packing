...
class LoadAwarePlacementControlsTest {
	...
	// -----------------------------------------------------------------------
	// populatePlacementSupporters
	// -----------------------------------------------------------------------

	/**
	 * Finds placements whose endZ equals point.minZ - 1 and that overlap the new
	 * box's footprint in the XY plane.
	 *
	 * <pre>
	 *  Point at z=1..5 (new box placed at z=1); supports must have endZ=0.
	 *
	 *  Y
	 *  |
	 * 10 +----------+  +----------+
	 *    |  A (ok)  |  |  C (far) |  A overlaps [0..9]×[0..9]; C is at x=20 → no overlap
	 *  0 +----------+  +----------+
	 *    0         10  20         30  X
	 *
	 *  B (endZ=1) is at the wrong z-level → excluded.
	 * </pre>
	 */
	@Test
	void testPopulatePlacementSupporters_filtersZAndOverlap() {
		// New box: 10×10, placed at (0,0,1); needs supports with endZ=0
		DefaultPoint3D point = new DefaultPoint3D(0, 0, 1, 20, 10, 5);
		BoxStackValue sv = Box.newBuilder()
				.withId("new").withSize(10, 10, 1).withWeight(1).withRotate2D()
				.build().getStackValues()[0];

		Placement a = placement("A", 10, 10, 1, 1, 0, 0, 0);  // endZ=0, overlaps → included
		Placement b = placement("B", 10, 10, 1, 1, 0, 0, 1);  // endZ=1, wrong level → excluded
		Placement c = placement("C", 10, 10, 1, 1, 20, 0, 0); // endZ=0, x=20..29, no XY overlap → excluded

		List<Placement> result = new ArrayList<>();
		LoadAwarePlacementControls.populatePlacementSupporters(point, sv, result, List.of(a, b, c));

		assertThat(result).containsExactly(a);
	}

	/**
	 * Two placements at the correct level, both overlapping the new box's footprint.
	 *
	 * <pre>
	 *  New box:  x=0..19, y=0..9  (20×10)
	 *  Support S1: x=0..9,  y=0..9  → full overlap on left half
	 *  Support S2: x=10..19, y=0..9 → full overlap on right half
	 *  Both have endZ = point.minZ - 1
	 * </pre>
	 */
	@Test
	void testPopulatePlacementSupporters_twoSupporters() {
		DefaultPoint3D point = new DefaultPoint3D(0, 0, 1, 20, 10, 2);
		BoxStackValue sv = Box.newBuilder()
				.withId("wide").withSize(20, 10, 1).withWeight(1).withRotate2D()
				.build().getStackValues()[0];

		Placement s1 = placement("S1", 10, 10, 1, 1, 0, 0, 0);
		Placement s2 = placement("S2", 10, 10, 1, 1, 10, 0, 0);

		List<Placement> result = new ArrayList<>();
		LoadAwarePlacementControls.populatePlacementSupporters(point, sv, result, List.of(s1, s2));
