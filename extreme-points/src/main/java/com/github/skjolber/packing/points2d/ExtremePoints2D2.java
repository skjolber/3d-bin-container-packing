...
public class ExtremePoints2D2<P extends Placement2D> implements ExtremePoints<P, Point2D> {
	...
	public ExtremePoints2D2(int dx, int dy) {
		setSize(dx, dy);
		addFirstPoint();
	}
	...
	protected void appendSwallowedOrShadowedAtXX(P placement, Point2D source, int xx, int yy, boolean includeSource) {
		...
		for (int i = 0; i < values.size(); i++) {
			...
			// Move points swallowed or shadowed by the placement
			if(point.shadowsOrSwallowsX(source.getMinX(), xx) && withinY(point.getMinY(), placement)) {
				if(point.getMaxX() > xx) {
