...
/**
 * Functional tests for {@link PressureConstraintBenchmark}.
 * <p>
 ...
 */
public class PressureConstraintBenchmarkTest {

	private static final File OUTPUT =
			new File("../visualizer/viewer/public/assets/containers.json");

	private PressureConstraintBenchmarkState state;
	private PressureConstraintBenchmark benchmark;

	@BeforeEach
	public void init() {
		state = new PressureConstraintBenchmarkState();
		state.init();
		benchmark = new PressureConstraintBenchmark();
	}
	...
	/** Two box types with different {@code maxLoadPressure} limits — both fit in one container. */
	@Test
	public void plainPackager_mixed() throws Exception {
		PackagerResult result = benchmark.packPlainMixed(state);
		assertTrue(result.isSuccess(), "Expected all mixed-pressure boxes to fit in one container");
		new DefaultPackagingResultVisualizerFactory(true)
				.visualize(result.getContainers(), OUTPUT);
	}

	/** Two box types with different {@code maxLoadPressure} limits — LAFF variant. */
	@Test
